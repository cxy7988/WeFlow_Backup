...
upload_generated_update_ymls() {
  ...
  if [ "$#" -eq 0 ]; then
    echo "No update yml asset names provided." >&2
    return 1
  fi

  if [ ! -d release ]; then
    echo "::error::release directory was not generated."
    return 1
  fi
  ...
  for asset_name in "$@"; do
    yml_path="$(find release -type f -name "$asset_name" -print -quit)"
    if [ -z "$yml_path" ]; then
      echo "::warning::$asset_name was not generated under release/."
      continue
    fi

    found="true"
    if ! grep -q '^minimumVersion:' "$yml_path"; then
      echo "minimumVersion: $minimum_version" >> "$yml_path"
    fi
    retry_cmd 5 3 gh release upload "$tag" --repo "$repo" "$yml_path" --clobber
  done

  if [ "$found" != "true" ]; then
    echo "::error::None of the expected update yml assets were generated: $*"
    echo "::group::Generated release files"
...
upload_generated_release_files() {
  local repo="$1"
  local tag="$2"
  ...
  while IFS= read -r asset_path; do
    assets+=("$asset_path")
  done < <(find release -maxdepth 1 -type f ! -name '*.yml' -print)

  if [ "${#assets[@]}" -eq 0 ]; then
    echo "::error::No release assets were generated under release/."
    echo "::group::Generated release files"
    find release -maxdepth 2 -type f -print || true
    echo "::endgroup::"
    return 1
  ...
  upload_release_assets_with_retry "$repo" "$tag" "${assets[@]}"
}
